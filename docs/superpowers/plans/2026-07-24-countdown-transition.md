...
- 碰撞专项：本局不计入上述 M1–M3。IAB fresh reload 后用鼠标点击启动，完整经过倒计时和正式游戏，随后刻意停止移动并以 100 毫秒间隔轮询状态等待碰撞；状态从“游戏开始”变为“游戏结束，坚持了14秒”。立即截图可见结算 overlay 后方、玩家原位置散开的橙色和棕色方块死亡粒子。页面本地既有最高分 87 秒保持正常，没有错误写入；页面 `warn` / `error` 日志均为空。
