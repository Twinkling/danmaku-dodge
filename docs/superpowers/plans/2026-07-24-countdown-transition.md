...
- 首刷、碰撞与结算证据：M2、K3 的 `5 秒`截图直接确认了自然首刷；其余四局没有单独记录敌人关键帧。全部六局最终均通过自然碰撞进入 gameover，并确认结算和重开，但没有单独捕捉“主动碰撞”或玩家死亡粒子关键帧，因此对应复选框不计完成。至少一次分别验证点击、Space、Enter 重开；结束文本可见，六局结算分别为 8、10、8、9、9、9 秒。页面本地既有最高分为 87 秒，本轮未刷新最高分，最高分显示保持正常。
- 记录完成状态：由于“主动碰撞并确认玩家死亡粒子关键帧”未完成，且精确 IAB 时间阈值截图不满足原视觉子项口径，不满足“上述验证全部完成”的条件；验证记录仍保留已实际完成的证据，对应汇总复选框保持未勾选。
...
- 控制台：六局和降级验证完成后，页面 `tab.dev.logs({ warn: true, error: true })` 均为空。控制工具自身的 Statsig 网络上报偶发 timeout 未出现在页面日志中，不计为页面问题。
