...
- [x] 浏览器中临时通过 DevTools 阻断离屏 `getImageData()` 后刷新，确认：
  - 页面不崩溃。
  - 显示实心数字。
  - 5.6 秒后仍进入正式游戏。
- [x] 撤销 DevTools 临时改动，确认正常粒子版本恢复；不把调试代码写入仓库。
...
- 自动化：fresh 执行 `npm run check`，实际结果为 76/76 通过、0 失败、0 跳过；其中覆盖离屏 Canvas 缺失和 `getImageData()` 抛错后的实心数字降级。另以测试名称筛选 fresh 执行离屏缺失专项，实际结果为 2/2 通过、0 失败。
...
- 浏览器主时序：真实点击后状态变为“倒计时开始”。连续定时截图中，开始后 0.2、0.7 秒均为白色方块组成的 `3`，HUD 为 `0 秒`、玩家居中、无敌人且无旧“准备”文案；1.4 秒时 `3` 的方块向外散成环；2.3 秒显示方块 `2`，3.0 秒时 `2` 向外散；4.1 秒显示方块 `1`。额外近景中，4.65 秒时 `1` 清晰停留，4.90 秒时青色 `1` 明显向玩家压缩，5.15 秒时仅确认粒子几乎回到玩家；外罩首次明确证据为 5.3 秒时已经形成，5.35 秒时数字粒子消失且保护罩更明显；5.75 秒时数字消失、保护罩完整，状态为“游戏开始”，HUD 从 `0 秒`开始正式计时。
