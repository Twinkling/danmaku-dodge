...
- [x] 用浏览器访问 `http://127.0.0.1:4174/`，不要使用 `file://` 作为最终验收入口。
...
- [x] 在自动化夹具中确认离屏 Canvas 缺失时的实心数字测试通过。
- [x] 浏览器中先 reload 清理页面缓存，在首次数字采样前通过 IAB CDP `Runtime.evaluate` 临时令离屏 `getImageData()` 抛错，再点击确认：
  - 页面不崩溃。
  - 显示实心数字。
  - 5.6 秒后仍进入正式游戏。
- [x] reload 移除临时原型改动，确认正常粒子版本恢复；不把调试代码写入仓库。

### Step 5：人工试玩

- [x] 鼠标完整试玩 3 局。
- [x] 键盘完整试玩 3 局。
- [x] 每局确认正式存活时间不包含 5.6 秒倒计时。
- [x] 至少一局坚持到正式保护结束并观察自然首刷。
- [x] 至少一局主动碰撞，确认玩家死亡粒子、结算和最高分正常。
...
- 自动化：fresh 执行 `npm run check`，实际结果为 76/76 通过、0 失败、0 跳过；其中浏览器入口测试精确覆盖 5.19 秒无外罩，以及 5.4、5.55 秒保护罩半径和透明度单调增加；同时覆盖离屏 Canvas 缺失和 `getImageData()` 抛错后的实心数字降级。另以测试名称筛选 fresh 执行离屏缺失专项，实际结果为 2/2 通过、0 失败。
- 静态与版本状态：`git diff --check` 退出码为 0；验证开始前 `git status --short --branch` 仅显示 `## feature/huanxiong/gameplay-pacing`，无未提交文件。`git diff --stat f390d59..HEAD` 为 11 个文件、3967 行新增、134 行删除；`git log --oneline f390d59..HEAD` 共 20 个提交，范围为 `94503ff` 至 `51a766f`。
- 服务：复用任务开始前已运行的 PID 32125（`python3 -m http.server 4174 --bind 127.0.0.1`），HTTP HEAD 返回 200。遵守约束未另启重复服务，因此 Step 2 的“启动本地 HTTP 服务”复选框保持未勾选；最终保留一个 4174 标签，页面处于“等待开始游戏”。
- 浏览器初始状态：在 IAB 中 claim 现有 `http://127.0.0.1:4174/` 标签并 reload；DOM snapshot 包含说明段落、`#game-status` 的“等待开始游戏”和唯一 `application`“弹幕躲避游戏”，页面正常绘制，初始 `warn` / `error` 日志均为空。
- 浏览器主时序：真实点击后状态变为“倒计时开始”。连续定时截图中，开始后 0.2、0.7 秒均为白色方块组成的 `3`，HUD 为 `0 秒`、玩家居中、无敌人且无旧“准备”文案；1.4 秒时 `3` 的方块向外散成环；2.3 秒显示方块 `2`，3.0 秒时 `2` 向外散；4.1 秒显示方块 `1`。额外近景中，4.65 秒时 `1` 清晰停留，4.90 秒时青色 `1` 明显向玩家压缩，5.15 秒时仅确认粒子几乎回到玩家；外罩首次明确证据为 5.3 秒时已经形成，5.35 秒时数字粒子消失且保护罩更明显；5.75 秒时数字消失、保护罩完整，状态为“游戏开始”，HUD 从 `0 秒`开始正式计时。另一轮 fresh 补拍的 early、mid、late 三个画面中，状态均为“倒计时开始”：early 可见白色 `1` 且无外罩，mid 可见青色 `1` 向玩家回流，late 可见外罩明显出现。
- 视觉完成状态：真实 IAB 已观察到 `3`、`2` 外散、`1` 回流，以及倒计时内从无外罩到外罩出现、正式开始时保护罩完整的广义视觉过程。但是 Browser 点击动作存在不可控的数百毫秒延迟，截图不能独立锚定精确的 5.2 秒阈值，也没有在同一轮 IAB 中取得 5.2–5.6 秒区间内两个确定时间点的截图。因此“验证视觉路径”整体不按原精确阈值口径计完成；精确边界由上述 fresh 自动化入口测试覆盖。
- 重复输入：fresh reload 后用鼠标启动，等待 2.0 秒时状态仍为“倒计时开始”；再次点击并按 Space、Enter，额外等待 3.8 秒后准时进入“游戏开始”。总等待未达到从重复输入重新计算的 5.6 秒，证明倒计时没有被重复输入重置。
- 试玩结论：六局均通过真实 UI 完成 `gameover / idle → countdown → running → gameover`，没有通过脚本篡改内部 state。倒计时不计入正式存活秒数；`3`、`2` 聚合、停留后外散，`1` 聚合、停留后回流；保护罩完全成形后才进入正式游戏。

| 路径 | 局次 | 启动与倒计时 | 正式移动与直接观察 | 结算与重开 |
| --- | --- | --- | --- | --- |
| 鼠标 | M1 | 从 gameover 点击重开，完整倒计时后状态为“游戏开始” | 移动到 `(90,180)`、`(430,700)`、`(260,180)`，玩家从中心移动到左上 | “游戏结束，坚持了8秒”；随后重开 |
| 鼠标 | M2 | 点击重开并完整经过倒计时与正式保护 | 依次移动到 `(430,160)`、`(100,700)`、`(420,650)`；正式 4.8 秒截图的 HUD 为 `5 秒`，可见绿色自然首刷敌人与右侧玩家 | “游戏结束，坚持了10秒”；随后重开 |
| 鼠标 | M3 | 点击重开并完整经过倒计时与正式保护 | 沿四角移动 | “游戏结束，坚持了8秒”；随后重开 |
| 键盘 | K1 | Space 重开并完整经过倒计时与正式保护 | ArrowLeft 24 次、ArrowUp 18 次，玩家明显从中心移到左侧 | “游戏结束，坚持了9秒”；随后重开 |
| 键盘 | K2 | Enter 重开并完整经过倒计时与正式保护 | `d` 20 次、`s` 14 次、`a` 15 次，玩家位置明显改变 | “游戏结束，坚持了9秒”；随后重开 |
| 键盘 | K3 | Space 重开并完整经过倒计时与正式保护 | ArrowRight、ArrowDown、ArrowLeft、ArrowUp 各 12 次并跨过正式 4.8 秒；HUD 为 `5 秒`，可见紫色自然首刷敌人与玩家 | “游戏结束，坚持了9秒”；随后通过输入验证重开 |

- 首刷与六局结算证据：M2、K3 的 `5 秒`截图直接确认了自然首刷；其余四局没有单独记录敌人关键帧。全部六局最终均通过自然碰撞进入 gameover，并确认结算和重开。至少一次分别验证点击、Space、Enter 重开；结束文本可见，六局结算分别为 8、10、8、9、9、9 秒。
- 碰撞专项：本局不计入上述 M1–M3。IAB fresh reload 后用鼠标点击启动，完整经过倒计时和正式游戏，随后刻意停止移动并以 100 毫秒间隔轮询状态等待碰撞；状态从“游戏开始”变为“游戏结束，坚持了14秒”。立即截图可见结算 overlay 后方、玩家原位置散开的橙色死亡粒子，部分因透明度衰减呈较暗色。页面本地既有最高分 87 秒保持正常，没有错误写入；页面 `warn` / `error` 日志均为空。
- 记录完成状态：主动碰撞、玩家死亡粒子、结算和最高分证据已由碰撞专项补齐；但精确 IAB 时间阈值截图仍不满足原视觉子项口径，且按任务约束复用了既有服务、未执行“启动本地 HTTP 服务”步骤，因此仍不满足“上述验证全部完成”的条件，对应汇总复选框保持未勾选。
- 真实降级：IAB CDP 支持 `Runtime.evaluate`，但不支持 `Page.addScriptToEvaluateOnNewDocument`。因此先 reload 清理页面状态，再通过 `Runtime.evaluate` 临时令 `CanvasRenderingContext2D.prototype.getImageData` 抛出 `Error`；真实点击后 0.7 秒截图显示普通实心数字 `3`，页面未崩溃且状态为“倒计时开始”；5.8 秒后状态为“游戏开始”，页面 `warn` / `error` 日志仍为空。随后 reload 移除临时原型改动，再次点击后 0.7 秒截图恢复白色方块粒子 `3`；最终再次 reload 回到“等待开始游戏”。调试代码未写入仓库。
- 控制台：六局、碰撞专项和降级验证完成后，页面 `tab.dev.logs({ warn: true, error: true })` 均为空。控制工具自身的 Statsig 网络上报偶发 timeout 未出现在页面日志中，不计为页面问题。
