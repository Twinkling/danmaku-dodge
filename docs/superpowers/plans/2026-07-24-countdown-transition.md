...
- 浏览器主时序：真实点击后状态变为“倒计时开始”。连续定时截图中，开始后 0.2、0.7 秒均为白色方块组成的 `3`，HUD 为 `0 秒`、玩家居中、无敌人且无旧“准备”文案；1.4 秒时 `3` 的方块向外散成环；2.3 秒显示方块 `2`，3.0 秒时 `2` 向外散；4.1 秒显示方块 `1`。额外近景中，4.65 秒时 `1` 清晰停留，4.90 秒时青色 `1` 明显向玩家压缩，5.15 秒时粒子几乎回到玩家并开始成罩，5.3 秒时粒子已回到玩家附近且外罩形成，5.35 秒时数字粒子消失且保护罩更明显；5.75 秒时数字消失、保护罩完整，状态为“游戏开始”，HUD 从 `0 秒`开始正式计时。
...
| 路径 | 局次 | 启动与倒计时 | 正式移动与自然首刷 | 结算与重开 |
| --- | --- | --- | --- | --- |
| 鼠标 | M1 | 从 gameover 点击重开，完整倒计时后状态为“游戏开始” | 移动到 `(90,180)`、`(430,700)`、`(260,180)`，玩家从中心移动到左上，并观察到自然敌人 | “游戏结束，坚持了8秒”；随后重开 |
| 鼠标 | M2 | 点击重开并完整经过倒计时与正式保护 | 依次移动到 `(430,160)`、`(100,700)`、`(420,650)`；正式 4.8 秒截图的 HUD 为 `5 秒`，可见绿色自然首刷敌人与右侧玩家 | “游戏结束，坚持了10秒”；随后重开 |
| 鼠标 | M3 | 点击重开并完整经过倒计时与正式保护 | 沿四角移动并观察到自然敌人 | “游戏结束，坚持了8秒”；随后重开 |
| 键盘 | K1 | Space 重开并完整经过倒计时与正式保护 | ArrowLeft 24 次、ArrowUp 18 次，玩家明显从中心移到左侧，并观察到自然敌人 | “游戏结束，坚持了9秒”；随后重开 |
| 键盘 | K2 | Enter 重开并完整经过倒计时与正式保护 | `d` 20 次、`s` 14 次、`a` 15 次，玩家位置明显改变，并观察到自然敌人 | “游戏结束，坚持了9秒”；随后重开 |
...
- 结算与最高分：至少一次分别验证点击、Space、Enter 重开；结束文本可见，六局结算分别为 8、10、8、9、9、9 秒。页面本地既有最高分为 87 秒，本轮未刷新最高分，最高分显示保持正常。
